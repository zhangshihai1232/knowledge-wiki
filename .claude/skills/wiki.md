...
所有确定性文件操作（创建文件、移动文件、更新 frontmatter、追加日志、状态计算）**必须**通过 `wiki-ops` CLI 工具执行，不得由 LLM 直接操作文件系统。

```bash
# 工具位置
tools/wiki-ops.sh <subcommand> [options]

# 查看所有子命令
tools/wiki-ops.sh help
```

Spec 文件中标记为 ⚙️ 的步骤均通过 `wiki-ops` 执行；标记为 🧠 的步骤由 LLM 执行语义推理。
