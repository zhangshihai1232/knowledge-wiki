...
Spec 中标记为 ⚙️ 的步骤（提案移动、日志追加、状态更新）**必须**通过 `wiki-ops` CLI 工具执行：

```bash
tools/wiki-ops.sh move-proposal <file> --to approved --reviewed-by <name> --approve-note <note>
tools/wiki-ops.sh move-proposal <file> --to rejected --reviewed-by <name> --rejection-reason <reason>
tools/wiki-ops.sh move-proposal <file> --to inbox  # reopen
tools/wiki-ops.sh count inbox
tools/wiki-ops.sh count review
tools/wiki-ops.sh consecutive-approve-count
tools/wiki-ops.sh append-log --spec promote --message <msg>
tools/wiki-ops.sh update-state
```

标记为 🧠 的步骤（审查信息汇总、AI 建议生成）由 LLM 执行。标记为 🤝 的步骤由人工做最终决策。
