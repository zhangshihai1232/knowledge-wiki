...
wiki check     # 结构性健康检查
wiki review    # 审查队列
wiki apply     # 执行已批准提案编译
wiki apply list # 查看已批准待编译队列
wiki taxonomy  # 查看稳定分类注册表与待吸收建议
...
1. `ask`：先做轻量 query classification（`domain / primary_type / subtype`），再做字段收窄与词面排序；JSON 输出带 `contract_version`、检索元信息和分类提示
2. `import`：按单个 JSON payload 一次性写入 source + proposal，并可同步写入 claims / extracted / dedup evidence；若出现未注册分类，会进入 taxonomy suggestion queue；`--input -` 可从 stdin 读入
3. `maintain`：输出结构健康、队列统计、taxonomy pending suggestions 与可选衰减动作
4. `taxonomy suggestions / taxonomy accept / taxonomy reject`：查看并吸收 `domain / primary_type / subtype` 的候选分类，不让 AI 直接改写正式 registry
5. `review --json / apply list --json / resolve --json`：供 agent 读取确定性队列，避免解析表格文本
