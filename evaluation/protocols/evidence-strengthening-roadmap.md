...
- 它虽然已经过若干复杂 simulation，但还缺少一个**正式、可重复、可量化的高复杂度/大数据量对抗 benchmark**
...
| L1.5：受控鲁棒性 | 在更高复杂度、更大样本量、更强失败注入下验证 fixed 仍稳定 | 新增 |
...
1. **Adversarial high-scale benchmark**
1. **真实日志回放实验**
2. **多评审者一致性实验**
...
1. 先补 **Adversarial high-scale benchmark**
2. 再做 **真实日志回放**
3. 再做 **多评审者一致性**
4. 最后做 **长周期稳定性**
...
- 对抗 benchmark 先回答“复杂度再上一个台阶后还稳不稳”
- 日志回放再回答“真实场景是否成立”
- 多评审再回答“判分是不是可靠”
- 长周期最后回答“会不会过几周就退化”
