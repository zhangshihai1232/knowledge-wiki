...
```bash
wiki internal frontmatter get canon/domains/ai/concepts/embedding.md last_updated
wiki internal frontmatter get-list canon/domains/ai/concepts/embedding.md sources
```
...
1. 更新 canon 页 frontmatter：
   ...
   last_compiled: <今日日期>
   ```
2. 由 runtime 在 canon 页末尾追加维护记录节（若不存在则新建）：
   ```markdown
   ## 维护记录
...
```bash
wiki internal frontmatter set canon/domains/ai/concepts/embedding.md last_updated "2026-04-09"
wiki internal frontmatter set canon/domains/ai/concepts/embedding.md staleness_days "0"
wiki internal frontmatter set canon/domains/ai/concepts/embedding.md last_compiled "2026-04-09"
wiki internal append-maintenance-record canon/domains/ai/concepts/embedding.md \
  --record "2026-04-09：内容经 alice 确认有效，无实质更新，staleness 重置"
wiki internal append-log --spec refresh \
  --message "页面: ai/concepts/embedding | staleness_days: 104→0 | 处理: 内容确认 | 维护者: alice"
```

> **LLM 职责**：生成维护记录内容。  
> **runtime 职责**：把维护记录写入 `## 维护记录` 节。
...
wiki internal create-proposal \
  --action archive \
  --target-page "ai/concepts/deprecated-tool" \
  --trigger-source "system:refresh" \
  --confidence low \
  --body-file /tmp/archive-proposal-body.md
```
