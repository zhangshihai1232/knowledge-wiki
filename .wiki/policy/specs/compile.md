...
compile **只处理可直接写入 canon 的知识提案**。`origin=lint-patrol` 的治理提案、以及仍处于缺口登记状态的 `query-writeback` proposal，不属于本 spec 的处理范围。
...
- 文件 frontmatter 中 `compiled` 字段为空或为 `false` 时视为待处理
- 仅当 proposal 满足以下条件时，才进入 compile 队列：
  - `origin != lint-patrol`
  - `target_page` 不以 `_system/` 开头
  - 若 `origin = query-writeback`，则 `trigger_source` 已替换为真实 `sources/...` 路径
...
- **update**：若页面 title 发生变化则同步更新正文条目，否则不改动
...
2. 本次 compile 过程中**无冲突**（冲突检测规则未触发）。
3. 本次 compile 的 `## 变更内容` 节包含至少 1 条**正文级**新增或修改（即不是仅更新 `sources`、`confidence`、`staleness_days` 等 frontmatter 元数据）。
...
将模板中的占位字段替换为 proposal 提供的内容，生成完整 canon 页后写入 `canon/domains/{target_page}.md`。
...
对 `update` 类型的 action，检查 canon 页中非目标节的内容与编译前一致（字符级比对）。对 `archive` 类型，检查内容未被清空。`create` 类型跳过此检查。
