...
**重要**：lint 规则中 L001–L005、L007–L011 均为确定性结构检查（⚙️），仅 L006 跨页矛盾检测需要语义理解（🧠）。`wiki internal scan` 命令可执行所有结构性规则，LLM 仅需补充 L006 的语义分析。
...
- **触发条件**：某一领域目录下页面总数 > 50
- **严重级别**：INFO
- **说明**：单一领域页面过多会导致导航困难，索引文件膨胀，建议按主题拆分出子域（reclassify），不是重命名领域
- **建议动作**：路由到 maintain spec Section 1（领域拆分），使用 `reclassify` 操作按主题将页面迁移到子域：
  ```bash
  # 示例：将 ai 领域的 rag 主题拆分到 ai/retrieval 子域
  wiki migrate plan --op reclassify --from domain=ai subtype=rag --to domain=ai collection=retrieval
  wiki migrate dry-run <plan_id>
  wiki migrate apply <plan_id>
  ```
  ⚠️ 注意：`rename-domain` 只改域名，不减少页面数，无法消除 L007。必须使用 `reclassify` 将页面分散到更细粒度的领域/集合。
  触发条件达到 maintain 阈值（L007 ≥ 1 个大域）时，自动路由到 maintain spec Section 1（领域分裂）
...
### L012 unclassified-page — 未分类页面

- **检查对象**：canon/ 下所有页面的 frontmatter `subtype` 字段
- **触发条件**：`subtype` 字段为空字符串、`null` 或字段完全缺失，且页面路径不以 `_system/` 开头
- **严重级别**：INFO
- **说明**：未分类页面在分类检索（按 subtype 过滤）中不可见，降低知识发现质量；大量未分类页面表明摄入流程中 Step 2.5 跨域决策未被完整执行
- **建议动作**：
  - 若 L012 条目 **< 5 条**：逐条补充 subtype，可通过 `wiki internal frontmatter set <path> subtype "<value>"` 直接修复
  - 若 L012 条目 **≥ 5 条**：触发 maintain spec Section 7（未分类批量修复），使用 `findMatchingPages` 的 `subtype_is_null` 过滤器批量识别并制定迁移计划：
    ```bash
    wiki migrate plan --op reclassify --filter subtype_is_null=true --dry-run
    ```
...
3. 并行执行以下检查：
   ...
   - **L012**：检查 `subtype` 字段是否为空、`null` 或缺失（路径以 `_system/` 开头的页面豁免）。
4. 将所有发现追加到问题缓冲区，记录：规则 ID、文件路径、严重级别、具体说明。

**CLI 执行（结构性规则 L001–L005, L007–L012）**：

```bash
# 执行所有结构性 lint 检查（含 L012 未分类检查）
...
`wiki internal scan` 自动执行 L001（孤立页面）、L002（过期页面）、L003（缺少来源）、L004（断裂引用）、L005（低置信度超期）、L007（域溢出）、L008（超期提案）、L009（缺少索引）、L010（类型路径不匹配）、L011（连续批准率）、L012（未分类页面）的检查。

> **LLM 职责**：仅执行 L006 跨页矛盾检测——需语义理解两个 canon 页面对同一事实是否表述不一致，`wiki internal scan` 不覆盖此规则。
...
```bash
wiki internal consecutive-approve-count
# → 输出数字，如: 5
```
...
```bash
wiki internal append-log --spec lint \
...
1. **覆盖率检查**：L001–L012 十二条规则均已被执行，且每条规则的检查结果（即使为零发现）均已写入报告。若任一规则因异常被跳过，报告该规则为 `[SKIPPED]` 状态并标记 Gate 失败。
