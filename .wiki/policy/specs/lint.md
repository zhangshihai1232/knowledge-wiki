...
- **检查对象**：changes/inbox/ 下所有提案文件
- **触发条件**：提案文件的创建时间距今超过 7 天且状态仍为 `pending`
- **严重级别**：WARNING
...
- **检查对象**：`changes/approved/` 和 `changes/rejected/` 目录下最近 20 条审查记录
- **触发条件**：最近 20 次 promote 决策中 approve 率 ≥ 90%（即 reject 率 < 10%）
- **严重级别**：WARNING
...
### Step 1 — 扫描 canon/ 页面（检查 L001、L002、L003、L004、L005、L006、L010）
...
### Step 3 — 扫描 changes/ 提案（检查 L008）

1. 遍历 `changes/inbox/*.md`，读取 frontmatter 中 `status` 和 `created_at` 字段。
2. 计算距今天数，若 `status == "pending"` 且超过 7 天，触发 L008。
3. 将发现追加到问题缓冲区。

### Step 3.5 — 扫描审查模式（检查 L011）

1. 读取 `changes/approved/` 和 `changes/rejected/` 目录下最近 20 条审查记录（按 `reviewed_at` 排序取最新 20 条）。
2. 计算 approve 率 = approved 数量 / 总审查数量。
3. 若 approve 率 ≥ 90%，触发 L011。
4. 将发现追加到问题缓冲区。
...
### Step 5.5 — 主动健康巡检与维护提案生成

在自动修复完成后，基于全量扫描结果评估系统健康趋势。当以下任一退化阈值触发时，自动在 `changes/inbox/` 生成维护提案：
...
trigger_source: "lint-auto-patrol"
confidence: high
proposed_at: "{today}"
...
[WARN]  L008 changes/inbox/update-pricing-logic.md — pending状态已持续9天
        → 建议：立即处理或拒绝，并通知相关维护者
