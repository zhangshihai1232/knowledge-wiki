...
以下阈值适用于所有 spec 中提到"AI 判断相似度"的场景：

| 场景 | 阈值 | 用途 |
|------|------|------|
| **页面级内容重叠**（dedup / merge） | **≥ 80%** | 两页内容高度重叠 → 触发 Step 3.3（合并）|
| **领域级结构重叠**（domain merge 建议） | **≥ 70%** | 两个领域知识边界模糊 → 建议 wiki migrate merge-subtype |

**测量代理（统一方法）**：  
LLM 对两段摘要+标签集合做二分类输出 `similar / different`，置信度 > 0.8 时采纳；  
或使用：`Jaccard(tag_set_A, tag_set_B)` 作为快速代理（无需语义模型）。

> 注：`maintain.md` 第 175 行的 70% 阈值适用于领域级；`reconcile.md` 第 97 行的 80% 阈值适用于页面级。两个数字均正确，含义不同，以上表为权威解释。
