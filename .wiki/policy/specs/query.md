  ...
  └── 列出所有领域（domains）及其简介
        └── canon/domains/{domain}/_index.md
              └── 列出该领域下所有 canon 页面的 slug 与摘要
                    └── canon/domains/{domain}/{slug}.md   ← 候选页面
```
...
3. 读取目标领域的 `_index.md`，获取该领域下的页面索引
...
- 用户问题答案中 `[⚠️ canon 外推断]` 标注的声明数量占全部事实性声明的比例超过 **25%**，说明 canon 覆盖度不足
...
- 事实性声明（定义、因果、数据、规范）必须 100% 带有来源标注（`[来源: slug]` 或 `[⚠️ canon 外推断]`）
